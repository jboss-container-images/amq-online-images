...
BROKER_PLUGIN_DIR=${BROKER_DIR}/broker-plugin
BROKER_CUSTOM=${BROKER_DIR}/custom
CONFIG_TEMPLATES=${BROKER_PLUGIN_DIR}/config_templates
...
cp -r ${BROKER_PLUGIN_DIR}/jmx_exporter $BROKER_CUSTOM
...
function pre_configuration() {
    ...
    JAVA_OPTS="-Dcom.sun.management.jmxremote=true -Djava.rmi.server.hostname=127.0.0.1 -Dcom.sun.management.jmxremote.port=1099 -Dcom.sun.management.jmxremote.ssl=true -Dcom.sun.management.jmxremote.registry.ssl=true -Dcom.sun.management.jmxremote.ssl.need.client.auth=true -Dcom.sun.management.jmxremote.authenticate=false -javaagent:/opt/amq/custom/jmx_exporter/jmx-exporter.jar=8080:/etc/prometheus-config/config.yaml"
